...
public class NotebotPlayer {
  ...
  public static boolean loadSong() {
    ...
    try {
      if (!mc.interactionManager.getCurrentGameMode().isSurvivalLike()) {
        ...
        mc.player.sendMessage(
            Text.literal("§6No song in queue!, Use §c/notebot queue add §6to add a song."));
    ...
    BlockPos playerEyePos =
        new BlockPos(
            (int) mc.player.getEyePos().x,
            (int) mc.player.getEyePos().y,
            (int) mc.player.getEyePos().z);

    List<BlockPos> noteblocks =
    ...
    if (required > blockPitches.size()) {
      mc.player.sendMessage(
          Text.literal(
              "§6Warning: Missing §c" + (required - blockPitches.size()) + " §6Noteblocks"));
    }
  ...
  public static void onTick(MinecraftClient client) {
    ...
    if (song == null) {
      ...
      NotebotPlayer.song =
          NotebotUtils.parse(
              NotebotFileManager.getDir().resolve("songs/" + NotebotPlayer.queue.remove(0)));
    ...
    for (Entry<BlockPos, Integer> e : blockPitches.entrySet()) {
      for (Note i : curNotes) {
        if (isNoteblock(e.getKey())
            && (i.pitch == getNote(e.getKey()))
            && (i.instrument == getInstrument(e.getKey()).ordinal())) playBlock(e.getKey());
      }
    }
  }
}
